...
| initImage | object | `{"pullPolicy":"IfNotPresent","repository":"apachesuperset.docker.scarf.sh/apache/superset","tag":null}` | Image used by the dependency-wait init containers. Defaults to the main Superset image so the startup-wait path reuses an already-required image instead of pulling a dedicated helper image (smaller deployment supply chain). |
